...
<?php if ( ! empty( $images ) ) { ?>
	...
	<div class="iconic-woothumbs-images-wrap">
		...
		<div class="iconic-woothumbs-images <?php if ( $this->settings['fullscreen_general_click_anywhere'] && $this->settings['fullscreen_general_enable'] ) {
			...
			foreach ( $images as $image ): ?>
				<div class="iconic-woothumbs-images__slide" <?php if ( $i > 0 ) {
					echo 'style="display: none;"';
				} ?> data-index="<?php echo $i; ?>">
					<?php do_action( 'iconic_woothumbs_image', $image, $i, $images ); ?>
				</div>
				<?php $i ++; endforeach; ?>
		</div>
	...
	<?php
	if ( $video_url ) {
		$video_orientation = Iconic_WooThumbs_Media::get_media_url_orientation( $video_url );
		?>
		<div id="iconic-woothumbs-video-template" style="display: none;">
			<div class="iconic-woothumbs-fullscreen-video-wrapper iconic-woothumbs-fullscreen-video-wrapper--<?php echo esc_attr( $video_orientation ); ?>">
