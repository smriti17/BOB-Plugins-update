	...
	var iconic_woothumbs = {
		...
		images_slider_args: function( product_object, index ) {
			...
			for (let i = 1; i < image_count; i++) {
				if (typeof product_object.images.children()[i] != "undefined") {
					var prod_data = String(product_object.images.children()[i].innerHTML);
					if (prod_data.toUpperCase().indexOf("SIZE") === -1 || prod_data.toUpperCase().indexOf("GUIDE") === -1) { 
						args.initialSlide = 1;
						image_changed = true;
						break;
					}
